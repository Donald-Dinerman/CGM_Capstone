...
logistic_gran_2hr_df <- raw_data %>%
  dplyr::select(TAR_bin_2h, respid, spring, tbh1female, weekend, tbh1pump,
                negative_interaction = frconfl, positive_interaction = frsupp, self_care = dmwell, 
                anxiety, happiness, depression = depr, atease, eat, exercise = exer
                ) %>%
  mutate(
...
logistic_2hr_df <- logistic_completeData_2hr %>%
  mutate(TAR_bin_2h_lagged = lag(TAR_bin_2h, n = 1, default = NA), .before = week) %>%
  drop_na(TAR_bin_2h_lagged)

# Fit Model
lag_2hr_logistic <- glm(TAR_bin_2h ~ TAR_bin_2h_lagged + time + week + spring + gender + pump + positive_interaction + negative_interaction +
                          self_care + anxiety + happiness + depression + atease + eat + exercise,
                        data = logistic_2hr_df, 
                        family = "binomial")

#Model Summary
summary(lag_2hr_logistic)

# Model Diagnostics
par(mfrow=c(2,2))
plot(lag_2hr_logistic)

# Residual Shape
hist(resid(lag_2hr_logistic))

# Multicollinearity
car::vif(lag_2hr_logistic)

acf2(resid(lag_2hr_logistic)) # ARMA(1,1)
```
...
lag_2hr_mm_logistic <- glmer(TAR_bin_2h ~ (1 | respid) + TAR_bin_2h_lagged + time + week + spring + gender + pump +
                               ...
                               atease + eat + exercise,
                             data = logistic_2hr_df, 
                             family = binomial, control = glmerControl(optimizer ="bobyqa")) # Nelder_Mead

# Model Summary
summary(lag_2hr_mm_logistic)

# print the mod results without correlations among fixed effects
print(lag_2hr_mm_logistic, corr = FALSE)

# Shape of residuals
resid_mm <- resid(lag_2hr_mm_logistic)
hist(resid_mm)
qqnorm(resid_mm); qqline(resid_mm)
acf2(resid_mm) # ARMA(1,1)
```
...
# Shape of residuals
fin_resid <- resid(lag_2hr_mm_logistic_error)
...
# Shape of residuals
resid_mm_pql <- resid(lag_2hr_mm_logistic_pql)
