...
```{r}
# Imputation
logistic_impute_list_2hr <- make.blocks(c("TAR_bin_2h"))

logistic_imputed_Data_2hr <- mice(logistic_gran_2hr_df, m=2, maxit = 50, method = 'logreg', seed = 500, blocks = logistic_impute_list_2hr)
...
```{r}
# Use binary response and respective lag
# 1 means they are above range
logistic_2hr_df <- int_fix_df %>%
...
# Model Summary
sum_mm_logistic <- summary(lag_2hr_mm_logistic)
...
beta_0 <- sum_mm_logistic$coefficients[8,1] # Effect of interaction for someone with "average" positive and negative interactions
beta_1 <- sum_mm_logistic$coefficients[9,1] # difference from the mean positive interaction
beta_2 <- sum_mm_logistic$coefficients[10,1] # difference from the mean negative interaction
