...
y_{itds} =& \; \alpha_{i} + \beta^{lag} y_{i(t-1)ds} + \beta^{time}x^{time}_{t} + \beta^{week} x^{week}_{d} + \beta^{season} x^{season}_{s} + \beta^{gender}x^{gender}_{i} +  

\\ &

\beta^{pump}x^{pump}_{is} +  \beta^{\:contact} x^{contact}_{itds} + \beta^{positive \:contact} x^{positive \:contact}_{itds} + \beta^{negative \:contact} x^{negative \:contact}_{itds} +

\\ &

\beta^{self\:care}x^{self\:care}_{itds} +  \beta^{anxiety}x^{anxiety}_{itds} + \beta^{happiness}x^{happiness}_{itds} + \beta^{depression}x^{depression}_{itds} +

\\ &

\beta^{at \:ease}x^{at \: ease}_{itds} +  \beta^{eat}x^{eat}_{itds} + \beta^{exercise}x^{exercise}_{itds} + \epsilon_{itds}
...
\beta^{pump}x^{pump}_{is} +  \beta^{\:contact} x^{contact}_{i(t-1)ds} + \beta^{positive \:contact} x^{positive \:contact}_{i(t-1)ds} + \beta^{negative \:contact} x^{negative \:contact}_{i(t-1)ds} +

\\ &

\beta^{self\:care}x^{self\:care}_{i(t-1)ds} +  \beta^{anxiety}x^{anxiety}_{i(t-1)ds} + \beta^{happiness}x^{happiness}_{i(t-1)ds} + \beta^{depression}x^{depression}_{i(t-1)ds} +

\\ &

\beta^{at \:ease}x^{at \: ease}_{i(t-1)ds} +  \beta^{eat}x^{eat}_{i(t-1)ds} + \beta^{exercise}x^{exercise}_{i(t-1)ds} + \epsilon_{i(t-1)ds}
...
\beta^{at \:ease}x^{at \: ease}_{itds} +  \beta^{eat}x^{eat}_{itds} + \beta^{exercise}x^{exercise}_{itds} + \beta^{int_1}x^{self\:care}_{itds}x^{anxiety}_{itds} + \beta^{int_2}x^{self\:care}_{itds}x^{happiness}_{itds} + 

\\ &

\beta^{int_3}x^{self\:care}_{itds}x^{depression}_{itds}+ \beta^{int_4}x^{self\:care}_{itds}x^{at \: ease}_{itds}
\epsilon_{itds}
...
gran_df <- raw_data %>%
  dplyr::select(TAR_prop_1h, respid, spring, weekend, tbh1female, weekend, 
...
gran_2hr_df <- raw_data %>%
  dplyr::select(TAR_prop_2h, respid, spring, tbh1female, weekend, 
                negative_interaction = frconfl, positive_interaction = frsupp, self_care = dmwell, 
                anxiety, happiness, depression = depr, atease, eat, exercise = exer
                ) %>%
  mutate(
...
```{r}
# Maybe treat as binary rather than continuous
gran_2hr_df %>%
  ggplot() +
  geom_histogram(aes(TAR_prop_2h), col = "Black", fill = "steelblue") +
  labs(x = "TAR 2hr", y = "# Responses", title = "TAR 2hr Distribution") +
  theme_bw()
```
...
```{r}
# Notes:
# Currently fitting a mixed model logistic using lmer and fitting a mm accounting for correlated errors using glmmPQL
  # Errors appear to be ARMA(1,1) ~ ARMA(0,1)
# Otherwise, I'm directly fitting a glmmPQL
  # Errors appear to be white noise
# Using glmmPQL achieves more normal residuals
```

```{r}
#table(raw_data$tbh2pump - raw_data$tbh1pump)

# Custom Pump Variable
fall_pump <- raw_data %>%
  dplyr::filter(spring == 0) %>%
  dplyr::select(respid, spring, tbh1pump)

spring_pump <- raw_data %>%
  dplyr::filter(spring == 1) %>%
  dplyr::select(respid, spring, tbh1pump = tbh2pump)

pump_comb <- rbind(fall_pump, spring_pump) %>% 
  rename(., pump_comb = tbh1pump) %>%
  distinct(respid, spring, pump_comb)

temp <- left_join(raw_data, pump_comb, by = c("respid", "spring"))

#dplyr::select(raw_data, respid, spring)
#dplyr::select(temp, respid, spring, pump_comb)

# Foreign keys: respid, spring
raw_data$mod_pump <- temp$pump_comb

# Set Data
logistic_gran_2hr_df <- raw_data %>%
  dplyr::select(TAR_bin_2h, respid, spring, tbh1female, weekend, mod_pump, interaction = int,
                negative_interaction = frconfl, positive_interaction = frsupp, self_care = dmwell, 
                anxiety, happiness, depression = depr, atease, eat, exercise = exer
                ) %>%
  mutate(
    TAR_bin_2h = as.factor(TAR_bin_2h),
    respid = as.factor(respid),
    time = time_diff(),
    week = weekend,
    spring = ifelse(spring==0, "Fall", "Spring"),
    gender = case_when(
      ...
      tbh1female == 1 ~ "Female",
      tbh1female == 2 ~ "Non Binary"),
    pump = as.factor(mod_pump),
    .keep = "unused", .after = TAR_bin_2h) %>%
  filter(gender != "Non Binary")

# Missing Values
logistic_gran_2hr_df %>%
  dplyr::select(everything()) %>%  # replace to your needs
  summarise_all(funs(sum(is.na(.))))
```

```{r}
# Fix Interaction missing data/interaction variable
# The two obs with missing interaction has missing positive/negative emotion
# Make them all zero
#raw_data$int %>% table()

#int_miss_ind <- is.na(raw_data$int) %>% which()

#int_df <- raw_data %>%
#  dplyr::select(int, negative_interaction = frconfl, positive_interaction = frsupp)

# int_df[int_miss_ind,]

# Make missing interactions zero
int_fix_df <- logistic_gran_2hr_df %>%
  mutate(interaction = coalesce(interaction, 0),
         positive_interaction = coalesce(positive_interaction, 0),
         negative_interaction = coalesce(negative_interaction, 0))
...
int_fix_df %>% 
  ...
  rename(., "Proportion Missing" = `.`) %>%
    mutate(Variable = case_when(
           Variable == "TAR_bin_2h" ~ "TAR",
           Variable == "springSpring" ~ "Spring",
           Variable == "genderMale" ~ "Male",
           Variable == "pump1" ~ "Pump",
           Variable == "self_care" ~ "Self Care",
           Variable == "atease" ~ "At Ease",
           Variable == "interaction" ~ "Contact",
           Variable == "positive_interaction" ~ "Positive Contact",
           Variable == "negative_interaction" ~ "Negative Contact",
...
```{r}
# Imputation for 10 times
logistic_impute_list_2hr <- make.blocks(c("TAR_bin_2h", "anxiety", "depression", 
                                          "atease", "self_care", "eat", "exercise"))

logistic_imputed_Data_2hr <- mice(int_fix_df, m=10, maxit = 50, method = 
                                    list(TAR_bin_2h ='logreg', anxiety = "pmm", 
                                         depression = "pmm", atease = "pmm", 
                                         self_care = "pmm", eat = "pmm",
                                         exercise = "pmm"), 
                                  seed = 500, blocks = logistic_impute_list_2hr)

# Can use the below command to see the first imputed data set
# logistic_completeData_2hr <- complete(logistic_imputed_Data_2hr, 1)

# table(raw_data$tbh1pump)
# table(raw_data$tbh2pump)

#dplyr::select(raw_data, respid, tbh1pump, tbh2pump)

# complete(logistic_imputed_Data_2hr, 1)

# complete(logistic_imputed_Data_2hr, 2)
```

```{r}
# Use a list to store all the original imputed data sets
imp_logistic_list <- list()

for (i in 1:10) {
  imp_logistic_list[[i]] <- complete(logistic_imputed_Data_2hr, i)
}
```

```{r}
# Use binary response and respective lag for all the imputed data sets
# 1 means they are above range
logistic_2hr_df <- list()
for (i in 1:10){
logistic_2hr_df[[i]] <- imp_logistic_list[[i]] %>%
  mutate(TAR_bin_2h = ifelse(TAR_bin_2h == 0, 1, 0), # Flip variable, to achieve more intuitive meaning
         TAR_bin_2h_lagged = lag(TAR_bin_2h, n = 1, default = NA), .before = week) %>%
  drop_na(TAR_bin_2h_lagged)}
```


```{r}
# Fit Non-lagged Mixed Model
# Run the model for all the imputed data sets using function map()
# The model for just the i_th data set can be written as 
# lag_2hr_mm_logistic <- glmer(TAR_bin_2h ~ (1 | respid) + TAR_bin_2h_lagged + time + week + spring + gender + pump + 
#                               interaction + positive_interaction + negative_interaction +
#                               self_care + anxiety + happiness + depression + 
#                               atease + eat + exercise,
#                             data = logistic_2hr_df[[i]], 
#                             family = binomial, control = glmerControl(optimizer ="Nelder_Mead")) # bobyqa
lag_2hr_mm_logistic <- map(logistic_2hr_df,
                           ~ glmer(TAR_bin_2h ~ (1 | respid) + TAR_bin_2h_lagged + time + week + 
                                     spring + gender + pump + interaction + positive_interaction +                                            negative_interaction + self_care + anxiety + happiness +
                                     depression + atease + eat + exercise, data = .,
                             family = binomial, control = glmerControl(optimizer ="Nelder_Mead"))) # bobyqa

# Combine the outcomes from 10 models using pool() function
lag_2hr_mm_logistic_pool <- pool(lag_2hr_mm_logistic)

# Model Summary
sum_mm_logistic <- summary(lag_2hr_mm_logistic_pool)

odds_func <- function(x){
  val <- exp(x)
  return(val)
}

interp_odds_func <- function(x){
  val <- exp(x) - 1
  return(val)
}

# print the mod results without correlations among fixed effects
# print(lag_2hr_mm_logistic, corr = FALSE)

# The model summary that can be used for just running model for one data set(no map(), no pool())
# sum_mm_logistic$coefficients %>% 
#   data.frame() %>% 
#   mutate(odds_diff = odds_func(Estimate), .before = Estimate) %>%
#   rownames_to_column(var = "var_name") %>%
#   rename(., se = Std..Error, p_val = Pr...z..) %>%
#     mutate(Significant = ifelse(p_val < 0.05, 1, 0),
#          val_lower = round(Estimate - 1.96*se, 3),
#          val_upper = round(Estimate + 1.96*se, 3),
#          odds_lower = round(odds_func(val_lower), 3),
#          odds_upper = round(odds_func(val_upper), 3),
#          odds_ci = paste("[",odds_lower, ", ", odds_upper,"]", sep=""),
#          var_name = case_when(
#            var_name == "TAR_bin_2h_lagged" ~ "TAR Lag",
#            var_name == "springSpring" ~ "Spring",
#            var_name == "genderMale" ~ "Male",
#            var_name == "pump1" ~ "Pump",
#            var_name == "self_care" ~ "Self Care",
#            var_name == "atease" ~ "At Ease",
#            var_name == "interaction" ~ "Contact",
#            var_name == "positive_interaction" ~ "Positive Contact",
#            var_name == "negative_interaction" ~ "Negative Contact",
#            TRUE ~ str_to_title(var_name)
#   )) %>%
#   dplyr::select(var_name, odds_diff, odds_ci, Estimate, p_val, Significant) %>%
#   rename(., Variable = var_name, `Odds` = odds_diff, `Odds CI` = odds_ci, `P Value` = p_val) %>%
#   mutate_if(is.numeric, round, digits = 3) %>%
#   kbl() %>%
#   kable_classic()

# The model summary for the pooled outcome
sum_mm_logistic %>% 
  data.frame() %>% 
  mutate(odds_diff = odds_func(estimate), .before = estimate) %>%
  rename(., se = std.error, p_val = p.value, var_name = term) %>%
    mutate(Significant = ifelse(p_val < 0.05, 1, 0),
         val_lower = round(estimate - 1.96*se, 3),
         val_upper = round(estimate + 1.96*se, 3),
         odds_lower = round(odds_func(val_lower), 3),
...
# y = beta_0 + beta_1 (x-x_bar) + beta_2(z-z_bar)
beta_0 <- sum_mm_logistic$coefficients[8] # Effect of interaction for someone with "average" positive and negative interactions
beta_1 <- sum_mm_logistic$coefficients[9] # difference from the mean positive interaction
beta_2 <- sum_mm_logistic$coefficients[10] # difference from the mean negative interaction

ggplot(logistic_2hr_df) +
  geom_point(aes(x = negative_interaction, y = positive_interaction))
```


```{r, comment=NA}
# glmer Diagnostics for single model
# Residuals
resid_mm <- residuals(lag_2hr_mm_logistic, type = "pearson") # response
Residuals <- resid_mm
hist(Residuals) # Shape
p <- acf2(Residuals) # Autocorrelation

plot(lag_2hr_mm_logistic, type="p")

# linearity assumption
probabilities <- predict(lag_2hr_mm_logistic, type = "response")
predicted.classes <- ifelse(probabilities > 0.5, "1", "0")

mydata <- logistic_2hr_df %>%
  na.omit() %>%
  dplyr::select(anxiety, atease, depression, happiness, negative_interaction, positive_interaction, time)
...
ggplot(new_data, aes(logit, predictor.value))+
  geom_point(size = 0.5, alpha = 0.5) +
  geom_smooth(method = "loess") + 
  theme_bw() + 
  facet_wrap(~predictors, scales = "free_y")

# Influential Points (0.5 threshold)
model.data <- augment(lag_2hr_mm_logistic) %>% 
  mutate(index = 1:n()) 

ggplot(data = model.data) + 
  geom_point(aes(x = index, y = .cooksd), alpha = 0.2) +
  labs(x = "Observation Index", y = "Cook's Distance", title = "Cook's Distance Plot") +
  theme_bw()

# Multicollinearity
car::vif(lag_2hr_mm_logistic)
```

```{r}
# Model Dignostics for the pooled outcome

# Get the mean residuals for all 10 models
res_each=NULL

for(i in 1:10){
  res_each=rbind(res_each,residuals(lag_2hr_mm_logistic[[i]], type = "pearson"))
  Residuals=colMeans(res_each)}

hist(Residuals, main = "Histogram of Residuals")
p <- acf2(Residuals)
# Influential Points (0.5 threshold)
# Cook's Distance Plot
augmented_data <- lapply(lag_2hr_mm_logistic, augment) %>%
  bind_rows()

model.data <- augmented_data %>% 
  mutate(index = 1:n()) 
ggplot(data = model.data) + 
  geom_point(aes(x = index, y = .cooksd), alpha = 0.2) +
  labs(x = "Observation Index", y = "Cook's Distance", title = "Cook's Distance Plot") +
...
```{r}
# data set for lagged covariate model
# Use binary response and respective lag for all the imputed data sets
# Use respective lag for specified covariates
# 1 means they are above range
lagged_logistic_2hr_df <- list()
for (i in 1:10){
lagged_logistic_2hr_df[[i]] <- imp_logistic_list[[i]] %>%
  mutate(TAR_bin_2h = ifelse(TAR_bin_2h == 0, 1, 0), # Flip variable, to achieve more intuitive meaning
         TAR_bin_2h_lagged = lag(TAR_bin_2h, n = 1, default = NA), 
         interaction_lagged = lag(interaction, n = 1, default = NA),
         anxiety_lagged = lag(anxiety, n = 1, default = NA),
         depression_lagged = lag(depression, n = 1, default = NA),
         happiness_lagged = lag(happiness, n = 1, default = NA),
         atease_lagged = lag(atease, n = 1, default = NA),
         positive_interaction_lagged = lag(positive_interaction, n = 1, default = NA),
         negative_interaction_lagged = lag(negative_interaction, n = 1, default = NA),
         self_care_lagged = lag(self_care, n = 1, default = NA),
         eat_lagged = lag(eat, n = 1, default = NA),
         exercise_lagged = lag(exercise, n = 1, default = NA),
         .before = week) %>%
  drop_na(TAR_bin_2h_lagged, interaction_lagged, anxiety_lagged, depression_lagged, happiness_lagged,
          atease_lagged, positive_interaction_lagged, negative_interaction_lagged,
          self_care_lagged, eat_lagged, exercise_lagged)}

```

```{r}
# Fit lagged covariate Model for all the data sets using map()
lagged_2hr_mm_logistic <-map(lagged_logistic_2hr_df, ~ glmer(TAR_bin_2h ~ (1 | respid) + 
                            TAR_bin_2h_lagged + time + week + spring + gender + pump + 
                              interaction_lagged + positive_interaction_lagged +
                              negative_interaction_lagged + self_care_lagged + anxiety_lagged +
                              happiness_lagged + depression_lagged + atease_lagged + eat_lagged +
                              exercise_lagged, data = ., family = binomial, 
                            control = glmerControl(optimizer ="Nelder_Mead"))) # bobyqa

# Combine the outcomes of 10 models using pool() function
lagged_2hr_mm_logistic_pool <- pool(lagged_2hr_mm_logistic)

# Model Summary
lagged_sum_mm_logistic <- summary(lagged_2hr_mm_logistic_pool)
```

```{r}
# Summary for pooled outcome
lagged_sum_mm_logistic %>% 
  data.frame() %>% 
  mutate(odds_diff = odds_func(estimate), .before = estimate) %>%
  rename(., se = std.error, p_val = p.value, var_name = term) %>%
    mutate(Significant = ifelse(p_val < 0.05, 1, 0),
         ...
         var_name = case_when(
           ...
           var_name == "genderMale" ~ "Male",
           var_name == "pump1" ~ "Pump",
           var_name == "interaction_lagged" ~ "Contact Lag",
           var_name == "anxiety_lagged" ~ "Anxiety Lag",
           var_name == "happiness_lagged" ~ "Happiness Lag",
           var_name == "depression_lagged" ~ "Depression Lag",
           var_name == "self_care_lagged" ~ "Self Care Lag",
           var_name == "eat_lagged" ~ "Eat Lag",
           var_name == "exercise_lagged" ~ "Exercise Lag",
           var_name == "positive_interaction_lagged" ~ "Positive Contact Lag",
           var_name == "negative_interaction_lagged" ~ "Negative Contact Lag",
           var_name == "self_care_lagged" ~ "Self Care Lag",
           var_name == "atease_lagged" ~ "At Ease Lag",
           TRUE ~ str_to_title(var_name)
  )) %>%
  dplyr::select(var_name, odds_diff, odds_ci, estimate, p_val, Significant) %>%
  rename(., Variable = var_name, `Odds` = odds_diff, `Odds CI` = odds_ci, `P Value` = p_val,
...
# Get the mean residuals for all 10 models
lagged_res_each=NULL

for(i in 1:10){
  lagged_res_each=rbind(lagged_res_each,residuals(lagged_2hr_mm_logistic[[i]], type = "pearson"))
  Residuals=colMeans(lagged_res_each)}

hist(Residuals, main = "Histogram of Residuals")
p <- acf2(Residuals)
# Influential Points (0.5 threshold)
# Cook's Distance Plot
augmented_data <- lapply(lagged_2hr_mm_logistic, augment) %>%
  bind_rows()
...
```{r}
# Self_care and mood interaction model
mood_2hr_mm_logistic <- map(logistic_2hr_df, ~ glmer(TAR_bin_2h ~ (1 | respid) + 
                            TAR_bin_2h_lagged + time + week + spring + gender + 
                            pump + interaction + positive_interaction + negative_interaction + 
                            self_care + anxiety + happiness + depression + atease + 
                            eat + exercise + self_care*anxiety + self_care * happiness + 
                            self_care * depression + self_care * atease, data = .,
                            family = binomial, control = glmerControl(optimizer ="Nelder_Mead")))# bobyqa

# Combine the outcomes of 10 models using pool() function
mood_2hr_mm_logistic_pool <- pool(mood_2hr_mm_logistic)

# Model Summary
mood_sum_mm_logistic <- summary(mood_2hr_mm_logistic_pool)
```

```{r}
# Summary for the pooled outcome
mood_sum_mm_logistic %>% 
  data.frame() %>% 
  mutate(odds_diff = odds_func(estimate), .before = estimate) %>%
  rename(., se = std.error, p_val = p.value, var_name = term) %>%
    mutate(Significant = ifelse(p_val < 0.05, 1, 0),
         ...
         var_name = case_when(
           ...
           var_name == "self_care" ~ "Self Care",
           var_name == "atease" ~ "At Ease",
           var_name == "self_care * anxiety" ~ "Self Care * Anxiety",
           var_name == "self_care * happiness" ~ "Self Care * Happiness",
           var_name == "self_care * depression" ~ "Self Care * Depression",
           var_name == "self_care * atease" ~ "Self Care * At Ease",
           TRUE ~ str_to_title(var_name)
  )) %>%
  dplyr::select(var_name, odds_diff, odds_ci, estimate, p_val, Significant) %>%
  rename(., Variable = var_name, `Odds` = odds_diff, `Odds CI` = odds_ci, `P Value` = p_val,
...
for(i in 1:10){
  mood_res_each=rbind(mood_res_each,residuals(mood_2hr_mm_logistic[[i]], type = "pearson"))
  Residuals=colMeans(mood_res_each)}

hist(Residuals, main = "Histogram of Residuals")
p <- acf2(Residuals)
# Influential Points (0.5 threshold)
# Cook's Distance Plot
augmented_data <- lapply(mood_2hr_mm_logistic, augment) %>%
  bind_rows()
...
fit_3 <- auto.arima(resid_mm, trace = T, seasonal = F, stepwise = F, approximation = F)
summary(fit_3)

ma1 <- fit_3$coef[1]

error_cor <- corARMA(value = ma1, p = 0, q = 1)

lag_2hr_mm_logistic_error <- glmmPQL(TAR_bin_2h ~ TAR_bin_2h_lagged + time + week + spring + gender + pump +
                                       positive_interaction + negative_interaction + self_care + anxiety + happiness + depression + 
                                       atease + eat + exercise, 
                                     random = ~ 1 | respid,
                                     data = logistic_2hr_df, 
                                     family = binomial(),
                                     correlation = error_cor)

pql_error_sum <- summary(lag_2hr_mm_logistic_error)

pql_error_sum$tTable %>% 
  data.frame() %>% 
  mutate(odds_diff = odds_func(Value), .before = Value) %>%
  rownames_to_column(var = "var_name") %>%
    mutate(significant = ifelse(p.value < 0.05, 1, 0),
         val_lower = round(Value - 1.96*Std.Error, 3),
         val_upper = round(Value + 1.96*Std.Error, 3),
         odds_lower = round(odds_func(val_lower), 3),
         odds_upper = round(odds_func(val_upper), 3),
         odds_ci = paste("[",odds_lower, ", ", odds_upper,"]", sep="")) %>%
  dplyr::select(var_name, odds_diff, odds_ci, Value, p.value, significant) %>%
  mutate_if(is.numeric, round, digits = 3) %>%
  kbl() %>%
  kable_classic()

# Shape of residuals
fin_fit <- predict(lag_2hr_mm_logistic_error, newdata = logistic_2hr_df, type = "response")
num_resp <- as.numeric(logistic_2hr_df$TAR_bin_2h) - 1

fin_resid <- residuals(lag_2hr_mm_logistic_error, type = "pearson") #fin_fit - num_resp

hist(fin_resid, breaks = 200)
qqnorm(fin_resid); qqline(fin_resid)
acf2(fin_resid)
```

```{r,eval=F}
# No correlated errors bc white noise originally
lag_2hr_mm_logistic_pql <- glmmPQL(TAR_bin_2h ~ TAR_bin_2h_lagged + time + week + spring + gender + pump +
                                       positive_interaction + negative_interaction + self_care + anxiety + happiness + depression + 
                                       atease + eat + exercise,
                                   random = ~ 1 | respid,
                                   data = logistic_2hr_df,
...
pql_sum <- summary(lag_2hr_mm_logistic_pql)
...
pql_sum$tTable %>% 
  ...
  rownames_to_column(var = "var_name") %>%
  mutate(significant = ifelse(p.value < 0.05, 1, 0)) %>%
  dplyr::select(var_name, odds_diff, Value, p.value, significant) %>%
  mutate_if(is.numeric, round, digits = 3) %>%
  kbl() %>%
  kable_classic()

# Shape of residuals
resid_mm_pql <- resid(lag_2hr_mm_logistic_pql, type = "pearson")

hist(resid_mm_pql)
qqnorm(resid_mm_pql); qqline(resid_mm_pql)
acf2(resid_mm_pql)

# Temporal autocorrelation logistic regression
# What is the error for a logistic regression
# Pump variable
  # TBH1 is fall, TBH2 is spring
  # Make into one "pump" variable and use it
# Math behind correlation in glmmPQL (send to prof)
```
...
# Time series regression
# Fit ols
# Get ARMA(p,q) of residuals
# Refit OLS with assumption that residuals follow ARMA(p,q) process
# Verify refit residuals are white noise

# Turn binary characters to binary numbers
# Everything needs to be numeric (one hot encoding for characters) for xreg (svd)
ts_reg_df <- completeData_2hr %>% 
  na.omit() %>%
  dplyr::select(TAR_prop_2h, time, week, spring, gender, positive_interaction, negative_interaction, 
                  self_care, anxiety, happiness, depression, atease, eat, exercise) %>%
  mutate(spring = ifelse(spring == "Fall", 1, 0),
         gender = ifelse(gender == "Male", 1, 0)) 

fit_1 <- lm(TAR_prop_2h ~ time + week + spring + gender + positive_interaction + negative_interaction +
              self_care + anxiety + happiness + depression + atease + eat + exercise,
            data = ts_reg_df)

summary(fit_1)
# looks like AR(1), ARMA(1,0)
plot(ts(resid(fit_1)))
acf2(resid(fit_1))
...
```{r,eval=F}
external_reg <- ts_reg_df %>%
  dplyr::select(-TAR_prop_2h) %>%
  as.matrix()

external_reg[,2] %>% unique()

TAR_prop_2h_no_na <- ts_reg_df %>%
  dplyr::select(TAR_prop_2h) %>%
  as.matrix()

fit_2 <- arima(TAR_prop_2h_no_na, order = c(1,0,0), xreg = external_reg)

summary(fit_2)

# Residuals not looking like white noise
plot(resid(fit_2))
acf2(resid(fit_2))
...
```{r,eval=F}
# automatic "best" arima model search to model error
# ARIMA(p,d,q)
fit_3 <- auto.arima(TAR_prop_2h_no_na, xreg = external_reg, trace = T, seasonal = F, stepwise = F, approximation = F)

# These fits are equivalent
# fit_4 <- arima(TAR_prop_2h_no_na, order = c(1,0,4), xreg = external_reg)
```

```{r,eval=F}
# Summarise results
# Residuals look more like white noise here
# Hard to get away from bimodal residuals
sum_3 <- summary(fit_3)
...
```{r,eval=F}
external_reg_male <- ts_reg_df %>%
  dplyr::filter(gender == 1) %>%
  dplyr::select(-TAR_prop_2h) %>%
  as.matrix()

TAR_prop_2h_no_na_male <- ts_reg_df %>%
  dplyr::filter(gender == 1) %>%
  dplyr::select(TAR_prop_2h) %>%
  as.matrix()

fit_3_male <- auto.arima(TAR_prop_2h_no_na_male, xreg = external_reg_male, trace = T, seasonal = F, stepwise = F, approximation = F)

summary(fit_3_male)

plot(resid(fit_3_male))
acf2(resid(fit_3_male))

qqnorm(resid(fit_3_male))
qqline(resid(fit_3_male))

hist(resid(fit_3_male))
```

#### Female

```{r,eval=F}
external_reg_female <- ts_reg_df %>%
  dplyr::filter(gender == 0) %>%
  dplyr::select(-TAR_prop_2h) %>%
  as.matrix()

TAR_prop_2h_no_na_female <- ts_reg_df %>%
  dplyr::filter(gender == 0) %>%
  dplyr::select(TAR_prop_2h) %>%
  as.matrix()

# Error in search.arima(x, d, D, max.p, max.q, max.P, max.Q, max.order,  :  No ARIMA model able to be estimated
# fit_3_female <- auto.arima(TAR_prop_2h_no_na_female, xreg = external_reg_female, trace = T, seasonal = F, stepwise = F, approximation = F)
# 
# summary(fit_3_female)
# 
# plot(resid(fit_3_female))
# acf2(resid(fit_3_female))
# 
# qqnorm(resid(fit_3_female))
# qqline(resid(fit_3_female))
# 
# hist(resid(fit_3_female))
```

### Bootstrap Results

```{r,eval=F}
# Bootstrap beta parameters from models with error ARMA process considered and with it not considered
# Make point estimate the estimate from the model on the original data
# Calculate confidence interval as the values between the 2.5% and 97.5% of the data

avg_est <- sum_3$coef
beta_name <- sum_3$coef %>% data.frame() %>% rownames()
df_coef <- matrix(NA, nrow = 1, ncol = length(beta_name)) %>% as.data.frame()
colnames(df_coef) <- beta_name
n <- nrow(ts_reg_df)

B <- 1000
for(i in 1:B){
  df_boot <- sample_n(data.frame(ts_reg_df), size = n, replace = T)

  external_reg_boot <- df_boot %>%
    dplyr::select(-TAR_prop_2h) %>%
    as.matrix()

  TAR_prop_2h_no_na_boot <- df_boot %>%
    dplyr::select(TAR_prop_2h) %>%
    as.matrix()

  fit_boot <- arima(TAR_prop_2h_no_na_boot, order = c(1,0,4), xreg = external_reg_boot)

  sum_boot <- summary(fit_boot)

  df_coef[i,] <- sum_boot$coef
}

conf_int <- 0
conf_sig <- 0

for(i in 1:ncol(df_coef)){
  
  center <- avg_est[i]
  stddev <- sd(df_coef[,i])
  n <- length(df_coef[,i])
  
  se <- qt(0.975, df=n-1)*stddev/sqrt(n)
  lower_bound <- (center - se) %>% round(4)
  upper_bound <- (center + se) %>% round(4)
  
  conf_int[i] <- paste("(", lower_bound, ", ", upper_bound, ")", sep = "")
  
  conf_sig[i] <- ifelse(lower_bound <= 0 & upper_bound >= 0, 0, 1)
}

boot_results <- data.frame(Coefficients = beta_name, Estimate = round(avg_est,5), `CI 95` = conf_int, Significance = conf_sig)

rownames(boot_results) <- NULL
...
hist_func <- function(df) {
  ...
  for (column in column_names) {
    hist(df[[column]], main = column, xlab = "")
  }
}
...
```{r,eval=F}
# Compare Bootstrap Confidence Intervals with Normal model Confidence Intervals

# Bootstrap mixed model with lag for response

# Wants mixed model with lag (previous mood) effects for mood/interactions on the response, don't include time t mood/interactions
```
...
avg_est_mm <- sum_mm$coefficients[,1]
beta_name_mm <- avg_est_mm %>% data.frame() %>% rownames()
df_coef_mm <- matrix(NA, nrow = 1, ncol = length(beta_name_mm)) %>% as.data.frame()
colnames(df_coef_mm) <- beta_name_mm

respid_df <- completeData_2hr %>%
  mutate(TAR_2hr_lagged = lag(TAR_prop_2h, n = 1, default = NA), .before = week) %>%
  drop_na(TAR_2hr_lagged)
...
B <- 1000
for(i in 1:B){
  df_boot_mm <- sample_n(data.frame(respid_df), size = n, replace = T)

  fit_boot_mm <- lmer(TAR_prop_2h ~ (1 | respid) + TAR_2hr_lagged + time + week + spring + gender + positive_interaction + negative_interaction + 
                    self_care + anxiety + happiness + depression + atease + eat + exercise,
                    data = df_boot_mm)

  sum_boot_mm <- summary(fit_boot_mm)

  df_coef_mm[i,] <- sum_boot_mm$coefficients[,1]
}

conf_int_mm <- 0
conf_sig_mm <- 0

for(i in 1:ncol(df_coef_mm)){
  ...
  conf_int_mm[i] <- paste("(", lower_bound, ", ", upper_bound, ")", sep = "")
  
  conf_sig_mm[i] <- ifelse(lower_bound <= 0 & upper_bound >= 0, 0, 1)
}

boot_results_mm <- data.frame(Coefficients = beta_name_mm, 
                              Estimate = round(avg_est_mm,5), 
                              `CI 95` = conf_int_mm, 
                              Significance = conf_sig_mm)

rownames(boot_results_mm) <- NULL
...
par(mfrow = c(2,2))
hist_func(df_coef_mm)
```

```{r,eval=F}
# Compare Bootstrap vs Normal
# OLS
boot_results %>% kbl() %>% kable_classic(full_width = F, html_font = "Cambria")
tab_model(fit_3)

# MM
boot_results_mm %>% kbl() %>% kable_classic(full_width = F, html_font = "Cambria")
tab_model(lag_2hr_mod)
```
