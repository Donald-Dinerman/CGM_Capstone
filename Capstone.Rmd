...
B <- 1000
for(i in 1:B){
  df_boot <- sample_n(data.frame(ts_reg_df), size = n, replace = F)
...
#apply(df_coef,2, hist, main = "t")

# Final Result
# Coefficient           Average Estimate          95% Confidence Interval

ci_func <- function(x, sig_ind = T){
  ...
  str <- paste("(", lower_bound, ", ", upper_bound, ")", sep = "")
  
  sig <- ifelse(lower_bound <= 0 & upper_bound >= 0, 0, 1)
  
  ifelse(sig_ind == F, return(str), return(sig))
}

avg_est <- apply(df_coef, 2, mean)

conf_int <- apply(df_coef, 2, ci_func, sig_ind = F)

conf_sig <- apply(df_coef, 2, ci_func, sig_ind = T)
