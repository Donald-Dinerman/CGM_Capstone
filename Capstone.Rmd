...
gran_df <- raw_data %>%
  dplyr::select(TAR_prop_1h, respid, emanum0, spring, tbh1female, datetime_ema, 
                positive_emotion = frconfl, negative_emotion = frsupp, self_care = dmwell, 
                anxiety, happiness, depression = depr, atease, eat, exercise = exer
                ) %>%
  mutate(
    respid = as.factor(respid),
    time = emanum0 + 1, # So it starts at 1 rather than 0 (Treat as numeric?)
...
y_imp_mod <- lmer(TAR_prop_1h ~ (1 | respid) + time + week + spring + gender + positive_emotion + negative_emotion + 
                    self_care + anxiety + happiness + depression + atease + eat + exercise,
                    data = completeData)
...
lag_mod <- lmer(TAR_prop_1h ~ (1 | respid) + TAR_1hr_lagged + time + week + spring + gender + positive_emotion + negative_emotion + 
                  self_care + anxiety + happiness + depression + atease + eat + exercise,
                data = lagged_df)
...
tab_model(gran_mod, y_imp_mod, show.ci = F)

# Imputed Y Models
tab_model(y_imp_mod, lag_mod, show.ci = F)
```
