...
plot_grid(p2_gen, p3_gen, p4_gen)
```
...
\beta^{mood} x^{mood}_{itds} + \beta^{positive \:emotions} x^{positive \:emotions}_{itds} +

\\ &

\beta^{negative \:emotions} x^{negative \:emotions}_{itds} + \beta^{self care}x^{self care}_{itds} + \beta^{eat}x^{eat}_{itds} + \beta^{exercise}x^{exercise}_{itds} + \epsilon_{itds}
...
y_{itds} =& \; \alpha_{i} + \beta^{lag} y_{i(t-1)ds} + \beta^{time}x^{time}_{t} + \beta^{week} x^{week}_{d} + \beta^{season} x^{season}_{s} + \beta^{gender}x^{gender}_{i} +  \beta^{mood} x^{mood}_{itds} +  \beta^{positive \:emotions} x^{positive \:emotions}_{itds} +
...
gran_df <- raw_data %>%
  dplyr::select(TAR_bin_1h, respid, emanum0, spring, tbh1female, datetime_ema, 
                frconfl, frsupp, 
                dmwell, anxiety, happiness, depr, atease, eat, exer
                ) %>%
  mutate(
    respid = as.factor(respid),
    emanum0 = emanum0 + 1, # So it starts at 1 rather than 0 (Treat as numeric?)
...
```{r}
# Drop all NA
# Do I use glmer to have categorical response (family = binomial)
# If so I get following warning:
  # Model failed to converge with max|grad| = 0.00776465 (tol = 0.002, component 1)
gran_mod <- lmer(TAR_bin_1h ~ (1 | respid) + emanum0 + week + spring + gender + frconfl + frsupp + dmwell + anxiety + happiness + depr + atease + eat + exer,
...
```{r}
# With TAR_bin_1h (y) Imputation
library("mice")

impute_list <- make.blocks(c("TAR_bin_1h"))

# Make y categorical
gran_df$TAR_bin_1h <- as.factor(gran_df$TAR_bin_1h)

imputed_Data <- mice(gran_df, m=1, maxit = 50, method = 'logreg', seed = 500, blocks = impute_list)
...
completeData$TAR_bin_1h <- as.numeric(completeData$TAR_bin_1h)
y_imp_mod <- lmer(TAR_bin_1h ~ (1 | respid) + emanum0 + week + spring + gender + frconfl + frsupp + dmwell + anxiety + happiness + depr + atease + eat + exer,
                    data = completeData)
...
fit <- with(data = gran_df, 
            exp = lmer(TAR_bin_1h ~ (1 | respid) + emanum0 + week + spring + gender + frconfl + frsupp + 
                         dmwell + anxiety + happiness + depr + atease + eat + exer))
...
lagged_df <- completeData %>%
  mutate(TAR_bin_1h = ifelse(TAR_bin_1h == 1, 0, 1),
         TAR_1hr_lagged = lag(TAR_bin_1h, n = 1, default = NA), .before = week) %>%
  drop_na(TAR_1hr_lagged)

lag_mod <- lmer(TAR_bin_1h ~ (1 | respid) + TAR_1hr_lagged + emanum0 + week + spring + gender + frconfl + frsupp + dmwell + anxiety +
                  happiness + depr + atease + eat + exer,
                data = lagged_df)
...
```{r}
# Levels matter for numeric if difference stays the same
table(completeData$TAR_bin_1h)
table(lagged_df$TAR_bin_1h)
table(gran_df$TAR_bin_1h)
```
