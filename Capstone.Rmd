...
resp_gen_df %>%
  ...
  ungroup() %>%
  group_by(gender) %>%
  summarise(bg_s_avg_1 = mean(bg_s_avg),
            bg_s_sd = sd(bg_s_avg, na.rm = T), # average of indivdual sd
...
mod_sum_df %>%
  ...
  summarise(m_con = mean(m_con, na.rm = T), m_str = mean(m_str, na.rm = T), m_ann = mean(m_ann, na.rm = T), 
            ...
            m_und = mean(m_und, na.rm = T), m_wel = mean(m_wel, na.rm = T),
...
library(lme4)
mixed_model <- lmer(m_TAR_s ~ m_con + m_str + m_ann + m_prob + m_sup + m_acc + m_fit + 
                      m_enj + m_tru + m_und + m_wel + as.factor(gender) + (1 | respid),
                    data = mod_sum_df)
...
# Also include the emotion variables and gender as covariate
```
